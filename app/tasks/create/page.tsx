...
export default function CreateTaskPage() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      const allowed = ranks.filter((r) => r.level < user.rank_level);
      if (allowed.length > 0) {
        setMinRank(allowed[0].level);
        setMaxRank(allowed[allowed.length - 1].level);
      }
    }
  }, []);

  const allowedRanks = currentUser
    ? ranks.filter((r) => r.level < currentUser.rank_level)
    : [];
  ...
  return (
    <main className="min-h-screen bg-black text-white p-10">
      ...
      <h1 className="text-4xl font-bold mb-8">發布任務</h1>
